...
Manual fitting for both tracked and full
...
def load_bar(iteration, total, prefix='', suffix='', decimal=1, length=100):
    percent = ('{0:.' + str(decimal) + 'f}').format(100 * (iteration/float(total)))
    filled_length = int(length*iteration//total)
    bar = r'>' * filled_length + r'-' * (length-filled_length)
...
def load_scatter(ribbon_num,which = 'C'):  
    ...
    x,y = x[mask3],y[mask3]
    print('{} out of {} events loaded'.format(len(x),length))
    return x,y

def window_load_scatter(ribbon_num,which = 'C'):  
    layer=(ribbon_num+50)//50
    ribbon=(ribbon_num%50)+1
    fn=f"../time_grad/GradientV3_{layer:0>2}_{ribbon:0>2}{which}"
    print(fn)
    if os.path.exists(fn):
        x, y = pickle.load(open(fn,'rb'))
        print('------ quick load complete------')
        return x,y
    else: 
...
# rx, ry=window_load_scatter(ribbon_ind, 'R')
...
# del(rx, ry)
# cx,cy=window_load_scatter(ribbon_ind, 'C')
...
def auto():
    ...
    with open('/home/genec420/full_manual/windowed_full_manual_record.csv', newline='') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            parameter_list.append(row)
    parameter_list = np.transpose(parameter_list)[1:]
    for raw_parameters in parameter_list:
        layer, ribbon, which = raw_parameters[0:3]
        print('#####################################  '+layer+'_'+ribbon)
        parameters=[0,0,0,0]
        for ind, i in enumerate(raw_parameters[3:]):
            if i!='':
        ...
        del(cx, cy)
        fig.tight_layout()
        fn = "TG_scatter_{}_{}".format(merge[ribbon_ind][0],merge[ribbon_ind][1])
        # fn = "scatter_{}_{}".format(merge[ribbon_ind][0],merge[ribbon_ind][1])
        plt.savefig(fn)
        print('\nget '+fn+".png \n")
        plt.close('all')
