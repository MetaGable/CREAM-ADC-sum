...
merge = pickle.load(open("/home/adetoito/manual_fitting/merge",'rb'))
data_type = input('track / full?\n')
# data_type = 'full'
def load_scatter(ribbon_num,which = 'C'):  
    ...
    if os.path.exists(file_prefix+str(ribbon_num)+str(which)):
        ...
        if which =='R':
            ...
            # input_type = 'home/adetoito/manual_fitting/gain2/no*'
        else:
            input_type = '/home/genec420/'+directory+'/C*'
            # input_type = 'home/adetoito/manual_fitting/gain2/C*'
    ...
    # retrieve basis for gradient using regex
    oldest_date = re.search('[0-9]{4}-[0-9]{2}-[0-9]{2}', input_paths[0]).group(0)
    if data_type == 'track':
        newest_date = re.search('[0-9]{4}-[0-9]{2}-[0-9]{2}', input_paths[-1]).group(0)
    elif data_type == 'full':
        newest_date = re.search('[0-9]{8}', input_paths[-1]).group(0)
        newest_date = newest_date[0:4] + '-' + newest_date[4:6] + '-' + newest_date[6:8]

    lower_bound = datetime.fromisoformat(oldest_date).timestamp()
    upper_bound = datetime.fromisoformat(newest_date).timestamp()
    interval = upper_bound - lower_bound

    for path in input_paths:
        input_data = pickle.load(open(path, 'rb'))

        # appends input data into scatter (1-dim)
        scatter[0]+=input_data[ribbon_num][1]
        scatter[1]+=input_data[ribbon_num][0]

        # retrieve gradient value dependent on time
        pattern1 = re.search('[0-9]{4}-[0-9]{2}-[0-9]{2}', path)
        if pattern1:
            date = pattern1.group(0)
        pattern2 = re.search('[0-9]{8}', path)
        if pattern2:
            date = pattern2.group()
            date = date[0:4] + '-' + date[4:6] + '-' + date[6:8]
        date_val = datetime.fromisoformat(date).timestamp()
            
        for i in range(0, len(input_data[ribbon_num][0])):
            gradient.append((date_val - lower_bound) / interval)
    scatter = np.array(scatter)
...
def data_cut(rib, ax, x, y, grad, fit, Cut_info, Slope_info):
    ...
    x,y,grad = x[mask],y[mask],grad[mask]
...
def plot_labels(ax, x, y, grad, title, low_x='', xlim='', tightcuts=False):
    ...
    _=ax.scatter(x, y, alpha = 0.6, s=13, c=grad, cmap='plasma')
    if tightcuts!=False:
...
ribbon_ind = 50 * (layer - 1) + (ribbon - 1)
...
# gain_list = [[] for i in range(1000)]
# gain_list = pickle.load(open("/home/adetoito/manual_fitting/gain2/plots/gain_table_L0", 'rb'))
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))
_=fig.suptitle('''{6} data - layer {0} ribbon {1}: ch{2} mid-range ADC over ch{3} low-range ADC
...
rx,ry,rgrad=load_scatter(ribbon_ind, 'R')
if plot1:
    ...
    x,y,grad = data_cut(ribbon_ind, ax1, rx, ry, rgrad, fit, L_Cut_info, L_Slope_info)
    fit_plot(ribbon_ind,ax1,x,y,fit,gain_list)
    del(x,y,fit)
plot_labels(ax1, rx, ry, rgrad, "With pedestal intact", Llow_x, Lxlim)
# plot_labels(ax1,rx, ry, "With pedestal intact", tightcuts=L_Cut_info)
del(rx, ry, rgrad)
cx,cy,cgrad=load_scatter(ribbon_ind, 'C')
if plot2:
    ...
    x,y,grad = data_cut(ribbon_ind, ax2, cx, cy, cgrad, fit, R_Cut_info, R_Slope_info)
    fit_plot(ribbon_ind,ax2,x,y,fit,gain_list)
    del(x,y,fit)
plot_labels(ax2, cx, cy, cgrad, "Subtracted pedestal with Cmean", Rlow_x, Rxlim)
# plot_labels(ax2, cx, cy, "Subtracted pedestal with Cmean", tightcuts=R_Cut_info)
del(cx, cy, cgrad)
fig.tight_layout()
fn = "scatter_{}_{}_{}manual".format(merge[ribbon_ind][0], merge[ribbon_ind][1], data_type[0].capitalize())
# plt.savefig(fn)
plt.savefig("manual_fit_plots/{}".format(fn))
print('#####################################\nget '+fn+".png \n#####################################")
plt.close('all')
